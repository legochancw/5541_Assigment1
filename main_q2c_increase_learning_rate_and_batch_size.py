...
if __name__ == '__main__':
    ...
    batch_size =64
    n_class = 10
    learning_rate = 0.1  # from 1e-3 increase to 0.1
    ...
    model_name = 'alexnet'
    model = AlexNet(n_class).to(device)
    num_epoch = 50
    # Criterion & Optimizer & Scheduler
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(model.parameters(), learning_rate, momentum, weight_decay)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=50, eta_min=0)
    scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer, learning_rate, num_epoch)
    ...
    for epoch in range(num_epoch):
        ...
        print(
            f'[{model_name}_batchSize] finish epoch {epoch + 1}, train loss: {train_loss:.4f}, test loss: {test_loss:.4f}, '
    ...
    torch.save({
        ...
        'loss': criterion,
    }, f'./outputs/{model_name}_batchSize_model.pth')
    ...
    plt.savefig(f'./outputs/{model_name}_batchSize_accuracy.jpg')
    plt.show()
    ...
    plt.savefig(f'./outputs/{model_name}_batchSize_loss.jpg')
    plt.show()
